 ...
 *
 * Uses WebAssembly (compiled from engine.c via Emscripten) to run
 * the simulation algorithms directly in the browser.
 *
 * Handles:
...
/**
 * Run the C engine via WebAssembly.
 * Passes CLI arguments to main(), captures stdout JSON output.
 *
 * @param {string[]} args — CLI arguments (e.g. ['--mode', 'contiguous', ...])
 * @returns {object} — Parsed JSON result from the engine
 */
async function runEngine(args) {
    ...
    const Module = await createEngineModule({
        ...
        printErr: (text) => { console.warn('Engine stderr:', text); }
    });

    try {
        Module.callMain(args);
    } catch (e) {
        // Emscripten may throw on exit(); that's fine
        if (typeof e !== 'number' && e !== 'unwind') {
            throw e;
        }
    }

    const trimmed = output.trim();
    if (!trimmed) throw new Error('Engine produced no output');
    return JSON.parse(trimmed);
}

/**
 * Simulate contiguous allocation via WASM.
 */
async function simulateContiguous(algo, holes, requests) {
    const args = [
        '--mode', 'contiguous',
        '--algo', algo,
        '--holes', holes.join(','),
...
/**
 * Simulate page replacement via WASM.
 */
async function simulatePaging(algo, frames, sequence) {
    const args = [
        '--mode', 'paging',
        '--algo', algo,
...
// Mark engine ready once WASM loads
(async () => {
    try {
        // Test that the module can be instantiated
        await createEngineModule({ noInitialRun: true, print: () => {}, printErr: () => {} });
        engineReady = true;
        const badge = document.getElementById('engine-status');
        if (badge) { badge.textContent = 'WASM Ready'; badge.className = 'badge badge-green'; }
    } catch (e) {
        console.error('WASM init failed:', e);
        const badge = document.getElementById('engine-status');
        if (badge) { badge.textContent = 'WASM Error'; badge.className = 'badge badge-red'; }
    }
})();
...
formContig.addEventListener('submit', async (e) => {
    ...
    try {
        const data = await simulateContiguous(algo, holes, requests);

        if (data.error) {
            renderContigError(data.error);
        } else {
            renderContigResult(data);
        }
    } catch (err) {
        renderContigError('Engine error: ' + err.message);
    } finally {
        btn.disabled = false;
...
formPaging.addEventListener('submit', async (e) => {
    ...
    try {
        const data = await simulatePaging(algo, frames, sequence);

        if (data.error) {
            renderPagingError(data.error);
        } else {
            pagingData = data;
            currentStep = 0;
        ...
        renderPagingError('Engine error: ' + err.message);
    } finally {
        btn.disabled = false;
        btn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polygon points="5 3 19 12 5 21 5 3"/></svg> Run Simulation`;
    }
...
function showStepsUpTo(stepNum) {
    ...
    for (let i = 0; i < pagingData.steps.length; i++) {
        const row = document.getElementById(`step-row-${i}`);
        if (!row) continue;
        row.className = i < stepNum ? (i === stepNum - 1 ? 'active-step reveal-step' : '') : 'hidden-step';
    }
}
...
document.getElementById('btn-step-next').addEventListener('click', () => {
    ...
    updateFrames(pagingData.steps[currentStep - 1], prevFrames);
    updateStepIndicator();
